...
<body>
    ...
    <div class="font-bold mb-1 mt-1">Jadwal Pemeriksaan: {{ \Carbon\Carbon::parse($item->updated_at)->locale('id')->isoFormat('D MMMM Y') }}</div>
    ...
    <table class="form-table">
        <tr>
            ...
            <td class="col-value">{{ $item->name }}</td>
        </tr>
        <tr>
            <td class="col-label">Jenis Kelamin</td><td class="col-titik">:</td>
            <td class="col-value">{{ ucfirst($item->jenis_kelamin) }}</td>
        </tr>
        <tr>
            <td class="col-label">NIM</td><td class="col-titik">:</td>
            <td class="col-value">{{ $item->nim }}</td>
        </tr>
        <tr>
            ...
            <td class="col-value">
                <table width="100%" style="margin:0; padding:0; border:none; border-collapse: collapse;">
                    <tr>
                        <td width="70%" style="padding:0;">{{ $item->tempat_tanggal_lahir }}</td>
                        <td width="30%" style="padding:0;">Usia: {{ $item->usia }} Tahun</td>
                    </tr>
                </table>
            </td>
        </tr>
        <tr>
            ...
            <td class="col-value">{{ $item->fakultas }} / {{ $item->prodi }}</td>
        </tr>
        <tr>
            <td class="col-label">Disabilitas</td><td class="col-titik">:</td>
            <td class="col-value">{{ $item->disabilitas }}</td>
        </tr>
    </table>
    ...
    <table class="form-table" style="margin-top: 0;">
        <tr>
            ...
            <td class="col-value">{{ $item->tinggi_badan }} cm</td>
        </tr>
        <tr>
            <td class="col-label">Berat Badan</td><td class="col-titik">:</td>
            <td class="col-value">{{ $item->berat_badan }} kg</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="col-value"><u>{{ $item->tekanan_darah }}</u> mmHg</td>
        </tr>
        <tr>
            ...
            <td class="col-value">
                @if($item->ishihara == '-')
                    <strong>( - )</strong>
                @elseif($item->ishihara == '+')
                    <strong>( + )</strong>
                @elseif($item->ishihara == 'parsial')
                    <strong>Parsial</strong>
                @else
        ...
        <tr>
            ...
            <td class="col-value"><u>{{ $item->lingkar_perut ?: '-' }}</u> cm</td>
        </tr>
        <tr>
            <td class="col-label">Gula Darah*</td><td class="col-titik">:</td>
            <td class="col-value"><u>{{ $item->gula_darah ?: '-' }}</u> mg/dL</td>
        </tr>
        <tr>
            <td class="col-label">Visus Mata*</td><td class="col-titik">:</td>
            <td class="col-value">
                @if($item->visus_mata == 'Normal')
                    <strong>Normal</strong>
                @elseif($item->visus_mata == 'Gangguan')
        ...
        <tr>
            ...
            <td class="col-value">{{ $item->riwayat_sakit ?: '-' }}</td>
        </tr>
        ...
        <tr>
            ...
            <td class="col-value">{{ $item->keluhan ?: '-' }}</td>
        </tr>
    </table>
    ...
    <div>
        ...
        <span style="{{ $item->kesimpulan != 'Layak' ? 'text-decoration: line-through; color: #6c757d;' : 'font-weight: bold;' }}">Layak</span> / 
        
        <span style="{{ $item->kesimpulan != 'Layak dengan Syarat' ? 'text-decoration: line-through; color: #6c757d;' : 'font-weight: bold;' }}">Layak Dengan Syarat</span> / 
        
        <span style="{{ $item->kesimpulan != 'Tidak Layak Mengikuti PKKMB' ? 'text-decoration: line-through; color: #6c757d;' : 'font-weight: bold;' }}">Tidak Layak Mengikuti PKKMB</span>
    ...
    <div class="mt-2">
        <table width="100%" style="border-collapse: collapse;">
            <tr>
                <td width="20%" style="vertical-align: top; line-height: 25px; padding-top: 0;">
                    <strong><u>REKOMENDASI*</u> :</strong>
                </td>
                <td width="80%" style="vertical-align: top; padding-top: 0;">
                    @if($item->rekomendasi)
                        <div style="line-height: 25px; border-bottom: 1px solid #000; min-height: 25px;">
                            {{ $item->rekomendasi }}
                        </div>
    ...
    <table class="ttd-table">
        <tr>
            <td style="text-align: left; padding-left: 20px; vertical-align: top;">
                Malang, {{ \Carbon\Carbon::parse($item->updated_at)->locale('id')->isoFormat('D MMMM Y') }}<br>
            ...
            <td style="text-align: right; padding-right: 50px; vertical-align: top;">
                ...
                Dokter Pemeriksa
                <div style="margin-top: 10px; margin-bottom: 10px;">
                    @if($item->id_dokter_acc)
                        <img src="data:image/svg+xml;base64,{!! base64_encode(QrCode::size(70)->generate(route('validasi.nakes', $item->id_dokter_acc))) !!}" width="70" height="70">
                    @else
                        <div class="ruang-ttd"></div>
                    @endif
                </div>
                ( <u>{{ $item->id_dokter_acc ? $item->dokter->name : '..................................................' }}</u> )
                @if($item->id_dokter_acc && $item->dokter->nip)
                    <br>NIP. {{ $item->dokter->nip }}
                @endif
            </td>
        </tr>
    </table>
